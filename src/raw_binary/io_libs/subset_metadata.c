...
11/12/2014   Gail Schmidt     Added resampling option to be copied
...
)
{
    ...
    for (i = 0; i < inmeta->nbands; i++)
    {
        ...
        outmeta->band[iband].resampling_option =
            inmeta->band[i].resampling_option;
...
)
{
    ...
    for (i = 0; i < nbands; i++)
    {
        ...
        outmeta->band[iband].resampling_option =
            inmeta->band[j].resampling_option;
        count = snprintf (outmeta->band[iband].short_name,
            sizeof (outmeta->band[iband].short_name), "%s",
